...
| **DDoS / API Exhaustion** | Attacker scripts automated requests to `/api/chat` to exhaust API quotas and cause financial charges. | **Two-Tier Rate Limiting**: The server applies a general rate limit of 100 requests per 15 minutes. Heavy AI-powered routes (`/api/chat` and `/api/tips`) are throttled further by a strict rate limiter of 50 requests per 15 minutes. |
...
| **CSRF & Unauthorized Access** | External origins attempt cross-origin requests to execute calculation operations on behalf of user cookies. | **Strict CORS (Cross-Origin Resource Sharing)**: Rejects all requests from origins not explicitly matching the local environment or authorized hosting domains. |
